...
window._ = _
function checkSameItem(arr1, arr2) {
  return !!arr1.find((i) => {
...
function getCampCanMovePiece(currentCamp) {
  ...
  // 敌方保护的棋子
  const enemySavePiecesPosition = _.flatten(
    _.pluck(enemyList.canMoveList, 'canSavePiecePositions'),
  )
  // 敌方可以消除我方的棋子
  const enemyRemovePiecesPosition = _.flatten(
    _.pluck(enemyList.canMoveList, 'canRemovePiecePositions'),
  )
  // 敌方可以移动的所有坐标
  const enemyCanMovePositions = _.flatten(
    _.pluck(enemyList.canMoveList, 'canMovePositions'),
  )

  // 获取我方可移动的棋子
  const ownerList = getCampCanMovePieceList(currentCamp)
  // 我方保护的棋子
  const ownerSavePiecesPosition = _.flatten(
    _.pluck(ownerList.canMoveList, 'canSavePiecePositions'),
  )
  // 我方可以消除敌方的棋子
  ...
  canMoveList.find((item) => {
    ...
    if (checkSameItem(item.canSavePiecePositions, enemyRemovePiecesPosition))
      return false
  ...
  enemyRemovePiecesPosition.find((position) => {
    ...
    return canMoveList.find((item) => {
      ...
      if (checkSameItem(item.canSavePiecePositions, enemyRemovePiecesPosition))
        return false
      const nextPiece = getCanSavePiecePosition(
        item.ownerPiece,
        item.canMoveBlankPieces,
        position,
        enemyCanMovePositions,
      )
      if (nextPiece) {
        canSavePiece = {
          canMovePositions: item.canMovePositions,
...
function getCampCanMovePieceList(currentCamp) {
  const ownerPieces =
    currentCamp === 1 ? Game.getRedPieces() : Game.getBlackPieces()
  ...
  for (const ownerPiece of ownerPieces) {
    ...
    if (canMovePieces.king) {
      ...
      canMoveList.push({
        ...
        // 保护的棋子位置
        canSavePiecePositions: canMovePieces.owners.map((item) =>
          item && item.position ? item.position.toString() : '',
        ),
        // 可以消除的棋子位置
        canRemovePiecePositions: canMovePieces.others.map((item) =>
          item && item.position ? item.position.toString() : '',
        ),
        // 受保护的棋子
        savedByPieces: [],
...
function handleSavedByPieces(canMoveList) {
  canMoveList.forEach((item) => {
    for (const item2 of canMoveList) {
      if (
        item2.canSavePiecePositions.indexOf(
          item.ownerPiece.position.toString(),
        ) > -1
      ) {
...
// 获取移动后可以将军或保护其他棋子的位置
function getCanSavePiecePosition(
  ownerPiece,
  canMoveBlankPieces,
  needSavePosition,
  enemyCanMovePositions,
  ...
  return canMoveBlankPieces.find((item) => {
    if (enemyCanMovePositions.indexOf(item.position.toString()) > -1)
      return false // 如果下一步有危险，则不行动
    ...
    if (needSavePosition) {
      // 保护的棋子位置
      const canSavePiecePositions = canMovePieces.owners.map((item) =>
        item && item.position ? item.position.toString() : '',
      )
...
function handlePosition(canMovePosition, currentCamp) {
  ...
  for (const position of canMovePosition) {
    ...
    } else if (currentCamp !== piece.camp) {
      // 敌方阵营
      if (piece.name === 'k') {
